        ...
        If (Test-Connection -CN $Computer -Count 2 -BufferSize 16 -Quiet) 
        {
            ...
            If (Test-Path \\$Computer\C$)
            {
                ...
                else
                {
                    ...
                    $HS = "C:\folder\HairyShaman\HairyShaman_v0.4.exe"
                    $ZH = "C:\folder\ZombieHunter\ZombieHunter_v2.exe"
                    ...
                    $shareLoc = "\\$runComputer\C$\Share"
                    $runFile = "run.bat"
                    $sigFile = "sigcheck.exe"
                    $autoFile = "autorunsc.exe"
                    $tcpFile = "Tcpvcon.exe"
                    $runcmd = "C:\folder\RunBat\run.bat"

                    If(!(Test-Path $CopyLoc\folder\$Computer)){ New-Item -type Directory -force $CopyLoc\folder | Out-Null }
                    # copy_file Runs System           CopyTo           CopyFrom 
                    . Copy_file 0 $Computer $CopyLoc\folder $HSLocation $HSFile "HairyShaman" $Script:HS_Bad_Log
                    . Copy_file 0 $Computer $CopyLoc\folder $ZHLocation $ZHFile "ZombieHunter" $Script:ZH_Bad_Log
                    . Copy_file 0 $Computer $CopyLoc\folder $shareLoc\RunBat $runFile "RunBat"
                    . Copy_file 0 $Computer $CopyLoc\folder $shareLoc\SigCheck $sigFile "SigCheck" 
                    . Copy_file 0 $Computer $CopyLoc\folder $shareLoc\Autoruns $autoFile "Autoruns"
                    . Copy_file 0 $Computer $CopyLoc\folder $shareLoc\TCPView $tcpFile "TCPView"
...
#444444444444444444444444444444444444444444444444444444
                    ...
                    $process = [WMICLASS]"\\$Computer\ROOT\CIMV2:win32_process"  
                    $RV = $process.Create($runcmd) | Select ProcessId,ReturnValue
                    If($RV.ReturnValue -eq 0) { 
                        Write-Host "Started runbat on $Computer with a PID of $($RV.ProcessID)" 
                        echo "Started runbat on $Computer with a PID of $($RV.ProcessID)" | Out-File $Script:Log_File -Append }
                    else
                    {                             
                        switch ($RV.returnvalue) {
                            0 { $value = "Successful Completion" | Out-File $Script:Log_File -Append } 
                        ...
                        Write-Host "Attempted runbat on $Computer but had issue - $($Value)"
                        echo "Attempted runbat on $Computer but had issue - $($Value)" | Out-File $Script:Log_File -Append }
